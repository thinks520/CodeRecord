...
// 前面的几个是示例，接收按键指令后没有明确所按的键值。
// 本实例响应按键指令，输出所按的键值虚拟码。
...
)
{
	switch(uMsg)
	{
	case WM_CHAR:   //当按键时，显示所按键值
		MessageBox(hwnd,"按键指令","Win32",0);
		char szChar[100];
		sprintf(szChar,"所按键的字符值是 %d", wParam); //转换按键值类型
		MessageBox(hwnd,szChar,"VC++",0);   //用对话框的形式显示按键的值
		break;
	case WM_PAINT:  //绘图
		HDC hDC;
		PAINTSTRUCT ps; 
